            ...
            <?php foreach ($products as $p): ?>
                <div class="product-card">
                    <a href="product.php?id=<?php echo $p['id']; ?>">
                        <img src="<?php echo $p['image'] ? SITE_URL . '/uploads/' . htmlspecialchars($p['image']) : SITE_URL . '/assets/img/placeholder.svg'; ?>" 
                    ...
                    <div class="info">
                        ...
                        <button class="btn-add" onclick="addToCart(<?php echo $p['id']; ?>, '<?php echo htmlspecialchars(addslashes($p['name'])); ?>', <?php echo $p['price']; ?>)"
                                <?php echo $p['stock'] < 1 ? ' disabled' : ''; ?>>
                            加入购物车
                        </button>
                    </div>
                </div>
...
function addToCart(id, name, price) {
    ...
    alert('已加入购物车');
    document.querySelector('.cart-link').innerHTML = '购物车 (' + cart.reduce((s,i)=>s+i.quantity,0) + ')';
}
</script>
